...
const tHighlightNoteDialog = useTranslations("Reading.full.HighlightNoteDialog");
  return (
  <Dialog open={open} onOpenChange={() => setSelectedHighlight(null)}>
    <DialogContent style={{ backgroundColor: '#1a2a2f', borderColor: '#93D333' }}>
      <DialogHeader>
        <DialogTitle className="text-white">{tHighlightNoteDialog("title")} Thêm ghi chú cho đoạn highlight</DialogTitle>
      </DialogHeader>
      <div className="space-y-4">
        ...
        <Textarea
          placeholder={tHighlightNoteDialog("placeholder")}
          value={highlightNote}
        ...
        <div className="flex justify-end space-x-2">
          <Button variant="outline" onClick={() => setSelectedHighlight(null)} className="text-gray-300 border-gray-500 hover:bg-gray-700">
            {tHighlightNoteDialog("cancel")}
          </Button>
          <Button onClick={saveHighlightNote} className="text-white" style={{ backgroundColor: '#93D333' }}>{tHighlightNoteDialog("save")}</Button>
        </div>
      </div>
    </DialogContent>
  </Dialog>
