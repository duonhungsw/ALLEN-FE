...
import { Exercise, LearningSkillData } from "@/types/learning/learningType";
...
export default function ListeningPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useLearningSkill("listening");

  const dictationExercises: Exercise[] =
    listeningData?.data?.map((item: LearningSkillData, index: number) => ({
  ...
  const filteredDictationExercises = (dictationExercises || []).filter(
    (exercise) => {
      const matchesSearch =
  ...
  const renderExerciseCard = (
    ...
    >
      <Link
        href={`/learning/listening/${type === "fill-in" ? "fill-in/" : ""}${
          exercise.id
        }`}
      >
        <Card
              ...
              <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                <span className="flex items-center">
                  ...
                  {type === "fill-in"
                    ? exercise.blanks
                    : exercise.sentences}{" "}
                  {type === "fill-in" ? "chỗ trống" : "câu"}
                </span>
                <Badge
                  className={getLevelColor(exercise.difficulty)}
                  style={{
                    backgroundColor: getLevelBgColor(exercise.difficulty),
                  }}
                >
                  {exercise.difficulty}
                </Badge>
        ...
        <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <Tabs value={selectedTab} onValueChange={setSelectedTab}>
            ...
            <TabsContent value="dictation" className="mt-6">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-3 text-[#142F50]">
                  Chọn chủ đề luyện nghe
                </h2>
                <p className="text-gray-600">
                  Chọn một chủ đề để bắt đầu luyện nghe với các bài học có cấu
                  trúc
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                  Array.from({ length: 6 }).map((_, index) => (
                    <Card
                      key={index}
                      className="border animate-pulse shadow-sm bg-white border-gray-200"
                    >
                      <CardContent className="p-6">
                  ...
                  filteredDictationExercises.map((exercise, index) =>
                    renderExerciseCard(exercise, "dictation", index)
                  )
                )}
              </div>
            </TabsContent>
            ...
            <TabsContent value="fill-in" className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                  ...
                  filteredFillInExercises.map((exercise, index) =>
                    renderExerciseCard(exercise, "fill-in", index)
                  )
                )}
              </div>
            </TabsContent>
              ...
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <h4 className="font-semibold text-sm sm:text-base text-[#142F50]">
                      🎧 Chép chính tả:
                    </h4>
